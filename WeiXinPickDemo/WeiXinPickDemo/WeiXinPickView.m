...
-(void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.selectedAssets.count > maxSelectNumber) {
        [self.selectedAssets removeLastObject];
        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle: [NSString stringWithFormat:@"你最多只能选择%d张照片",maxSelectNumber] message:nil delegate:self cancelButtonTitle:@"我知道了" otherButtonTitles:nil, nil];
        [alertView show];
        return;
    }
    if (self.selectedAssets.count > 0) {
        NSString *string = [NSString stringWithFormat:@"发送  (%lu张）",(unsigned long)self.selectedAssets.count];
        self.rowData = @[@[string,@"从相册选择"],@[@"取消"]];
    }else{
        self.rowData = @[@[@"拍摄",@"从相册选择"],@[@"取消"]];
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 0) {
        if (indexPath.row == 0) {
            if (self.selectedAssets.count >0 ) {
                if (self.clickBlock) {
                    self.clickBlock(self.selectedAssets,PickPictureSendPicture);
                }
            }else{
                if (self.clickBlock) {
